...
-   その他のオプション
    ...
    -   `-pidfile {filename}` - プロセスIDの記録。ログローテションのために SIGHUPを送る対象
...
クエリを未指定で GET すると `/ui/`  (簡易UI)へリダイレクトする。
...
これを厳密に共有ディレクトリとプライベートディレクトリだけにアクセスできるようにするには、 `enable_external_access` を `false` に設定してください。
